...
using ..FESetCorotBeamModule: FESetL2CorotBeam, local_frame_and_def!, local_mass!, local_stiffness!, MASS_TYPE_CONSISTENT_WITH_ROTATION_INERTIA
...
function _buffers(self)
    return self._ecoords0, self._ecoords1, self._edisp1, self._dofnums, self._F0, self._Ft, self._FtI, self._FtJ, self._Te, self._elmat, self._elmatTe, self._aN, self._dN, self._DN, self._RI, self._RJ
end
...
function FEMMCorotBeam(integdomain::IntegDomain{S, F}, material::MatDeforElastIso) where {S<:FESetL2CorotBeam, F<:Function}
    ...
    _RI = fill(0.0, 3, 3)
    _RJ = fill(0.0, 3, 3)
    return FEMMCorotBeam(integdomain, material, _ecoords0, _ecoords1, _edisp1, _dofnums, _F0, _Ft, _FtI, _FtJ, _Te, _elmat, _elmatTe, _aN, _dN, _DN, _RI, _RJ)
end
...
function mass(self::FEMMCorotBeam, assembler::ASS, geom0::NodalField{FFlt}, u1::NodalField{T}, Rfield1::NodalField{T}, dchi::NodalField{T}; mass_type=MASS_TYPE_CONSISTENT_WITH_ROTATION_INERTIA) where {ASS<:AbstractSysmatAssembler, T<:Number}
    ...
    ecoords0, ecoords1, edisp1, dofnums, F0, Ft, FtI, FtJ, Te, elmat, elmatTe, aN, dN, DN, R1I, R1J = _buffers(self)
